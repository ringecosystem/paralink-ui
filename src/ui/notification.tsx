...
const createItem = (config: Config, status: Status, onClose: () => void) => {
  ...
  node.className = `border-radius border border-primary p-middle lg:p-5 flex items-center gap-middle mb-middle animate-notification-enter relative bg-component w-[82vw] lg:w-96 ${config.className}`;
  ...
  root.render(
    <>
      ...
      <div className="flex flex-col gap-small">
        {config.title && <div className="break-all text-xs font-bold lg:text-sm">{config.title}</div>}
        {config.description && <div className="break-all text-xs font-light lg:text-sm">{config.description}</div>}
      </div>
      {config.closable && (
        <button
          onClick={onClose}
...
const notificationInstance = (config: Config, status: Status) => {
  ...
  const { node: thisItem, root: thisRoot } = createItem(config, status, () => {
